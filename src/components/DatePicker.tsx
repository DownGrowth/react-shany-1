...
export const DatePicker: React.FC<Props> = (props) => {
  ...
  return (
    <div>
      ...
      <div flex children-grow-1 text-center children-p-8px>
        <span>年</span>
        <span>月</span>
        <span>日</span>
        <span>时</span>
      ...
      <div flex>
        ...
        <Column className="grow-1" items={hoursList} value={valueTime.current.day}
          onChange={(v) => { valueTime.current.hours = v; update({}) }} />
        <Column className="grow-1" items={minutesList} value={valueTime.current.day}
...
export const Column: React.FC<ColumnProps> = (props) => {
  ...
  return (
    <div className={className} h="50vh" overflow-hidden relative
      ...
      onTouchEnd={() => {
        const reaminder = translateY % itemHeight
        let y = translateY - reaminder
        if (Math.abs(reaminder) >= itemHeight / 2) {
          y += itemHeight * (reaminder > 0 ? 1 : -1)
        }
        setTranslateY(y)
        setIsTouching(false)
